...
function update(data) {
  ...
  circles.exit().attr("fill", "red").transition(t).attr("cy", y(0)).remove();
  ...
  circles
    ...
    .attr("cy", y(0))
    .attr("r", 5)
    .merge(circles) // AND UPDATE old elements present in new data
    .transition(t)
    .attr("cx", (item) => x(item.month) + x.bandwidth() / 2)
    .attr("width", x.bandwidth())
